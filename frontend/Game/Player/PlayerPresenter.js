...
export default class PlayerPresenter {
  ...
  handleInput(input) {
    ...
    // ジョイスティックの左右傾きで回転速度を決定
    if (joystick?.active) {
      // joystick.angle: 0が上、±πで左右
      // 右に傾けると正、左に傾けると負
      const rotationSpeed =
        -joystick.angle * joystick.strength * C.JOYSTICK_SENSITIVITY * 0.08; // 0.08は調整値
      ...
      // 現在のクォータニオンに回転を加算
      const nextQuat = currentQuat.clone().multiply(rotationDelta).normalize();
  ...
  updateDebugMonitor(playerModel, ballModel) {
    ...
    if (this.debugPlayerPos) {
      ...
      this.debugPlayerPos.textContent = `x:${toFixed(pos.x)} z:${toFixed(
        pos.z
      )}`;
    }
